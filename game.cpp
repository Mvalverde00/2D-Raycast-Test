...
void Game::update(){
	...
	if (adt >= TICK_RATE) {
		adt -= TICK_RATE;
		screenManager.update(TICK_RATE, &mouseX, &mouseY);
	}
}

void Game::render(){
	SDL_SetRenderDrawColor(window.renderer, CONSTANTS::BLACK.r, CONSTANTS::BLACK.g, CONSTANTS::BLACK.b, SDL_ALPHA_OPAQUE);
